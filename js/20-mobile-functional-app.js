(function(){
  ...
  function show(screen, push=true){
    ...
    if(screen === 'social') setTimeout(function(){ try{ if(window.URBIS_PRESENCIA) window.URBIS_PRESENCIA.montarTarjeta(); }catch(e){} }, 100);
  ...
  // El catálogo de reportes rápidos se mudó a js/03c para que una app ligera
  // pueda usarlo sin cargar este archivo de 369 KB. Aquí se toman los mismos
  // nombres del objeto ya publicado, así el resto del archivo no cambia.
  const URBIS_QUICK_REPORT_SECTIONS = window.URBIS_QUICK_REPORT_SECTIONS || [];
  const URBIS_QUICK_REPORTS = window.URBIS_QUICK_REPORTS || {};
  const MANDATORY_PHOTO_IDS = window.URBIS_MANDATORY_PHOTO_IDS || new Set();
