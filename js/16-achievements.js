...
(function() {
  ...
  const ACHIEVEMENTS = [
    ...
    make('rep_first_validated','🚨','Primer reporte validado','Publique un reporte que sea aprobado por administración.','Reportes','Común', stats => stats.approvedReports >= 1, { requiresApproval: true }),
    ...
    // Actividad física
    make('move_first','🚶','Primer paso urbano','Registre su primera caminata, trote o bici.','Movimiento','Común', stats => stats.activities >= 1),
    make('move_1k','🏘️','Caminante de barrio','Complete 1 km acumulado.','Movimiento','Común', stats => stats.distanceKm >= 1),
    make('move_3k','🛣️','Ruta de 3K','Complete 3 km acumulados.','Movimiento','Común', stats => stats.distanceKm >= 3),
    make('move_5k','🌿','Cinco kilómetros de vida','Complete 5 km acumulados.','Movimiento','Poco común', stats => stats.distanceKm >= 5),
    make('run_4k','⚡','Trote de 4K','Complete 4 km trotando.','Movimiento','Poco común', stats => stats.runKm >= 4),
    make('run_8k','🔥','Trote de 8K','Complete 8 km trotando.','Movimiento','Raro', stats => stats.runKm >= 8),
    make('run_12k','🏅','Trote de 12K','Complete 12 km trotando.','Movimiento','Épico', stats => stats.runKm >= 12),
    make('move_week_3','📆','Constancia semanal','Registre 3 salidas en una semana.','Movimiento','Poco común', stats => stats.weeklyActivities >= 3),
    make('move_week_5','💪','Semana activa','Registre 5 actividades en una semana.','Movimiento','Raro', stats => stats.weeklyActivities >= 5),
    ...
    make('move_morning','🌅','Ruta temprana','Registre una actividad antes de las 8 a. m.','Movimiento','Poco común', stats => stats.morningActivities >= 1),
    make('move_night','🌙','Ruta nocturna','Registre una actividad después de las 6 p. m.','Movimiento','Poco común', stats => stats.nightActivities >= 1),
    make('move_park','🌳','Vuelta al parque','Registre una ruta cerca de un parque.','Movimiento','Poco común', stats => stats.parkActivities >= 1),
    make('move_personal_best','📈','Mi mejor distancia','Supera su distancia personal anterior.','Movimiento','Raro', stats => stats.personalBests >= 1),

    // Bienestar
    make('mood_first','😊','Primer check-in emocional','Registre su estado de ánimo por primera vez.','Bienestar','Común', stats => stats.moodLogs >= 1),
    make('mood_3','🫶','Me escuché hoy','Registre cómo se siente 3 días.','Bienestar','Común', stats => stats.moodLogs >= 3),
    make('mood_7','📔','Diario de ánimo','Registre ánimo 7 veces.','Bienestar','Poco común', stats => stats.moodLogs >= 7),
    make('mood_week','🌈','Semana consciente','Registre ánimo todos los días de una semana.','Bienestar','Raro', stats => stats.moodStreak >= 7),
    make('mood_30','🗓️','Mes emocional','Registre ánimo 30 veces.','Bienestar','Épico', stats => stats.moodLogs >= 30),
    make('mood_low','💬','Reconocer también cuenta','Registre un día de bajo ánimo.','Bienestar','Común', stats => stats.lowMoodLogs >= 1),
    make('mood_move','🧠','Mente y movimiento','Registre actividad física y ánimo el mismo día.','Bienestar','Poco común', stats => stats.moodAndMoveDays >= 1),
    make('mood_better_move','🌤️','Mejor después de moverme','Registre mejor ánimo después de caminar o trotar.','Bienestar','Raro', stats => stats.betterAfterMove >= 1),
    make('mood_gratitude','🙏','Día de gratitud','Escriba una nota positiva.','Bienestar','Poco común', stats => stats.gratitudeNotes >= 1),
    make('mood_selfcare','🧡','Cuidé de mí','Registre una actividad de descanso o autocuidado.','Bienestar','Poco común', stats => stats.selfcareLogs >= 1),

    // Eventos
    make('event_first_view','🎪','Primer evento visto','Abra la ficha de un evento.','Eventos','Común', stats => stats.eventViews >= 1),
    make('event_first_attend','🙋','Asistí a uno','Marque asistencia a un evento.','Eventos','Común', stats => stats.eventsAttended >= 1),
    ...
    make('event_route','🧭','Ruta al evento','Use una ruta hacia un evento.','Eventos','Poco común', stats => stats.eventRoutes >= 1),
    make('event_share','📣','Reto compartido','Comparta un evento o reto comunitario.','Eventos','Raro', stats => stats.eventShares >= 1),
    make('event_20','🏆','Embajador del barrio','Participa en 20 eventos.','Eventos','Legendario', stats => stats.eventsAttended >= 20),

    // Minijuegos
    make('game_first','🎮','Primer intento gamer','Juegue su primer minijuego.','Juegos','Común', stats => stats.gamesPlayed >= 1),
    ...
    make('game_top10','🏘️','Top del barrio','Entre al top 10 de un ranking barrial.','Juegos','Raro', stats => stats.top10Count >= 1),
    ...
    make('game_mountain','🚗','Montaña URBIS','Juegue el minijuego de montaña.','Juegos','Común', stats => stats.mountainGames >= 1),
    make('game_personal_best','🔁','Sin rendirse','Mejora su propio puntaje anterior.','Juegos','Raro', stats => stats.gamePersonalBests >= 1),
    ...
    make('profile_created','👤','Perfil creado','Complete su perfil básico.','Perfil','Común', stats => stats.profileComplete >= 1),
    make('profile_photo','🖼️','Foto lista','Suba foto de perfil.','Perfil','Común', stats => stats.profilePhoto >= 1),
    make('profile_phone','📱','Celular verificado','Registre o verifique su número celular.','Perfil','Común', stats => stats.phoneVerified >= 1),
    make('profile_theme','🎨','Estilo propio','Cambia el tema de la app.','Perfil','Común', stats => stats.themeChanges >= 1),
    make('profile_7days','🟢','Ciudadano activo','Entre a la app 7 días distintos.','Perfil','Poco común', stats => stats.loginDays >= 7),
    make('profile_30days','🔐','Usuario constante','Entre a la app 30 días distintos.','Perfil','Épico', stats => stats.loginDays >= 30),
    make('explore_first','📍','Primer lugar descubierto','Abra un punto del mapa.','Exploración','Común', stats => stats.mapPointsOpened >= 1),
    make('explore_10','🧭','Explorador de Cúcuta','Explore 10 puntos del mapa.','Exploración','Poco común', stats => stats.mapPointsOpened >= 10),
    make('explore_layer_3','🔍','Mapa curioso','Active 3 capas distintas.','Exploración','Poco común', stats => stats.layersActivated >= 3),
    make('explore_favorite','⭐','Punto importante','Guarde un lugar como favorito.','Exploración','Poco común', stats => stats.favorites >= 1),
    make('edu_mode','🎓','Modo educativo activado','Active permisos educativos.','Educativo','Común', stats => stats.eduModeActivated >= 1),
    make('edu_raster','🧩','Aprendiz GIS','Genera su primer raster o mapa de calor.','Educativo','Raro', stats => stats.rastersGenerated >= 1),
    make('gov_mode','🏛️','Rol comunitario activado','Active modo JAC o funcionario.','JAC','Común', stats => stats.govModeActivated >= 1),
    make('gov_review','🧾','Primer reporte revisado','Revise o aprueba un reporte ciudadano.','JAC','Común', stats => stats.reportsReviewed >= 1),
  ...
  const EXTRA_TIERS = [
    ...
    ['mood_60','🌻','Dos meses consciente','Registre ánimo 60 veces.','Bienestar','Legendario','moodLogs',60],
    ...
    ['explore_25','🗺️','Veinticinco lugares','Explore 25 puntos del mapa.','Exploración','Raro','mapPointsOpened',25],
    ['explore_50','🧭','Cartógrafo ciudadano','Explore 50 puntos del mapa.','Exploración','Épico','mapPointsOpened',50]
  ];
