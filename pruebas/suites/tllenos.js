...
(async()=>{
  ...
  const r=await pg.evaluate(async (D)=>{
    ...
    // La barra de andenes reusa el mismo marcado: se mira la PRIMERA, que
    // es la de llenos y vacíos.
    ...
    // ── Verlos en el mapa
    const bMapa=[...H.querySelectorAll('[data-pcr="llenos-mapa"]')][0];
    o.hayVerLlenos=!!bMapa;
    if(bMapa){
      ...
      // Y se pueden quitar
      const bQuitar=document.querySelector('#pcr-hoja [data-pcr="llenos-mapa"]');
      if(bQuitar){ bQuitar.click(); await esperar(400); }
      o.huellasTrasQuitar=document.querySelectorAll('.leaflet-overlay-pane path').length;
      // Se vuelve a poner para lo que sigue
      const bAg=document.querySelector('#pcr-hoja [data-pcr="agrandar"]');
      if(bAg){ bAg.click(); await esperar(300); }
    }
  ...
  P('se pueden quitar', r.huellasTrasQuitar < r.huellasEnMapa,
    r.huellasEnMapa+' → '+r.huellasTrasQuitar+' formas');
