...
(async () => {
  ...
  const t = await pg.evaluate(() => {
    ...
    const b = URBIS_CURSO.tipoDe('taller 5b  ufps', 'ana.perez', '7.8891,-72.5000|500');   // mismo curso, otras mayúsculas
  ...
  const rec = await pg.evaluate(async () => {
    ...
    const vieja = URBIS_CURSO.armar(null, { general: 'La de antier.' }, { lat: 7.8891, lng: -72.5 }, 500, 'Taller 5B', 'ana.perez');
    ...
    return { n: lista.length, autores: lista.map(x => x.autor),
             primeroTexto: (lista.find(x => x.autor === 'ana.perez') || {}).lecturas };
  });
