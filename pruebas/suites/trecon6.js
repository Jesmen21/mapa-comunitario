...
(async()=>{
  ...
  const r = await pg.evaluate(async (D) => {
    ...
    /* El botón tiene que estar donde se pregunta por el estrato —en el bloque
       de población— y no solo enterrado al final de la ficha: ahí no lo
       encontraba nadie. */
    o.btnEnPoblacion=!!h.querySelector('.pcr-estratos-btn');
    o.btnTexto=(h.querySelector('.pcr-estratos-btn')||{textContent:''}).textContent.replace(/\s+/g,' ').trim();
    (h.querySelector('.pcr-estratos-btn')||h.querySelector('[data-pcr="estratos"]')).click();
    ...
    o.manzanasPintadas=document.querySelectorAll('.leaflet-overlay-pane path').length;
    {
      ...
      if(q){ q.click(); await new Promise(r=>setTimeout(r,900)); }
      o.manzanasTrasQuitar=document.querySelectorAll('.leaflet-overlay-pane path').length;
      // Se vuelven a poner para lo que sigue comprobando la suite
      const ag=document.querySelector('#pcr-hoja [data-pcr="agrandar"]');
      if(ag){ ag.click(); await new Promise(r=>setTimeout(r,300)); }
      const otra=document.querySelector('#pcr-hoja .pcr-estratos-btn') ||
                 document.querySelector('#pcr-hoja [data-pcr="estratos"]');
      if(otra){ otra.click(); await new Promise(r=>setTimeout(r,1400)); }
      const ag2=document.querySelector('#pcr-hoja [data-pcr="agrandar"]');
  ...
  P('y está donde se pregunta por el estrato', r.btnEnPoblacion, r.btnTexto||'no está');
  P('al pintarlas la hoja baja para dejarlas ver', r.hojaBaja);
  P('la barra de abajo dice qué se está mirando', /Manzanas por estrato/.test(r.barraDice||''), r.barraDice);
  P('con su leyenda a la vista', r.leyendaEnBarra);
  P('y se pueden quitar', r.manzanasTrasQuitar < r.manzanasPintadas,
    r.manzanasPintadas+' → '+r.manzanasTrasQuitar+' formas');
